<template>
  <span class="nand-switch-container" :size="size" @click.stop="handleClick">
    <input class="nand-switch-input" :name="name" :value="modelValue" />
    <div
      class="nand-unselect nand-form-switch"
      :style="styles"
      :class="{
        'nand-form-onswitch': isActive,
        'nand-switch-disabled': disabled
      }"
    >
      ...
      <span>
        <div>
          <template v-if="loading">
            <i class="nand-icon nand-anim nand-anim-rotate nand-anim-loop" :class="loadingIcon"></i>
          </template>
          <template v-else>
...
<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<SwitchProps>(), {
    ...
    onswitchValue: true,
    unswitchValue: false,
    loadingIcon: 'nand-icon-loading-one',
    size: 'md'
  });
