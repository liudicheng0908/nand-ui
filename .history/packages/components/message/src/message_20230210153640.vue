...
<template>
  <transition name="tas-message-fade" @before-leave="onclose" @after-leave="$emit('destroy')">
    ...
    >
      <nand-icon :name="iconName" class="tas-message__icon" v-if="showIcon" />
      <slot>
        <span class="tas-message__content">{{ message }}</span>
      </slot>
      <nand-icon name="cross" class="tas-message__close-btn" v-if="closeable" @click="handleClose" />
    </div>
  </transition>
</template>
