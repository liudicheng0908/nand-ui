<template>
  <div v-click-outside class="nand-datepicker">
    <nand-input v-model="nowTime" :prefix-icon="prefixIcon" placeholder="Pick a day" />
    <div v-if="isVisible" class="nand-picker">
      <div class="nand-picker__header">
      ...
      <div class="nand-picker__weeks">
        <span v-for="(w, index) in weeks" :key="w">{{ w }}</span>
      </div>
      <div v-for="(m, index) in 6" :key="m" class="nand-picker__content">
        <span
          v-for="(n, index) in 7"
          :key="n"
          :class="{
...
<script setup lang="ts">
  ...
  import { ref, onBeforeMount, DirectiveBinding, reactive, computed, onMounted, watch } from 'vue';
